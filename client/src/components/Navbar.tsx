...
export default function Navbar({ onAuthOpen, onCartOpen }: NavbarProps) {
  ...
  // Mobile pills
  const [mobilePillsVisible, setMobilePillsVisible] = useState(true);
  const [mobileScrolled, setMobileScrolled] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const mobileScrollTimer = useRef<ReturnType<typeof setTimeout>>();

  // Track screen size to conditionally render desktop header
  ...
  // Desktop: hide while scrolling, show when stopped
  useEffect(() => {
  ...
  // Mobile pills: show when scrolling stops, hide while scrolling
  useEffect(() => {
    const onScroll = () => {
      setMobileScrolled(window.scrollY > 20);
      setMobilePillsVisible(false);
      clearTimeout(mobileScrollTimer.current);
      mobileScrollTimer.current = setTimeout(() => {
        setMobilePillsVisible(true);
      }, 150);
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => {
  ...
  return (
    <>
      {/* ══════════════════════════════════════
          DESKTOP NAV (xl and above) — fixed header
          Not rendered at all on mobile
      ══════════════════════════════════════ */}
      {isDesktop && (
      <header
      ...
      >
        ...
        <div className="flex container items-center justify-between h-16">
          {/* Logo */}
      ...
      {/* ══════════════════════════════════════
          MOBILE FLOATING PILLS (below xl)
          Just two floating pill buttons, no bar
      ══════════════════════════════════════ */}
      {/* Left: ywee branding pill */}
      <a href="/"
        className="xl:hidden fixed top-3 left-3 z-50 flex items-center gap-0.5 px-4 py-2 rounded-full text-sm font-bold shrink-0 shadow-lg transition-all duration-300"
        style={{
          ...
          fontSize: "15px",
          transform: mobilePillsVisible ? "translateY(0)" : "translateY(-120%)",
          opacity: mobilePillsVisible ? 1 : 0,
        }}>
      ...
      {/* Right: action pill */}
      <div
        className="xl:hidden fixed top-3 right-3 z-50 flex items-center gap-0.5 px-2 py-1.5 rounded-full shadow-lg transition-all duration-300"
        style={{
